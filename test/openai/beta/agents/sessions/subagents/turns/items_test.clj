(ns openai.beta.agents.sessions.subagents.turns.items-test
...
(def ^:private target-ns 'openai.beta.agents.sessions.subagents.turns.items)
...
(defn- agent-message ^AgentSessionItem []
  (AgentSessionItem/ofAgentMessage
   (-> (AgentSessionItem$AgentMessage/builder)
       (.id "item_1")
       (.addOutputTextContent "turn output")
...
(deftest validates-and-builds-turn-item-list-params
  (let [build-params (target-var '->item-list-params)]
    (is build-params "turn item list params builder is not implemented")
    (when build-params
...
(deftest invokes-turn-item-service-and-normalizes-items
  (let [item-list (target-var 'item-list)]
    (if item-list
      (let [item (agent-message)
            ...
            _ (reset! empty-page-holder
                      (-> (ItemListPage/builder)
                          ...
                          (.response (-> (ItemListPageResponse/builder)
                                         ...
                                         (.build)))
                          (.build)))
            turns (proxy [TurnService] [] (items [] items))
        ...
        (is (= [{:id "item_1"
                 :content [{:text "turn output" :type :output-text}]
                 :recipient-agent-id "agent_parent"
                 :sender-agent-id "agent_child"
                 :turn-id "turn_1"
                 :type :agent-message}]
               (item-list client "sess_1" "sub_1" "turn_1" {:limit 1})))
        (is (= 1 (opt (.limit ^ItemListParams @captured)))))
      (is false "turn item-list is not implemented"))))
